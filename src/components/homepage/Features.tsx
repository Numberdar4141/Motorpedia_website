...
function Features() {
  return (
    <section className="  bg-[#07205D] bg-[url(/images/counter_bg.png)]  stp-30 sbp-30">
          ...
          {featuresCard.map(({ id, icon, name, desc }, idx) => (
            <div className="col-span-12  sm:col-span-6 lg:col-span-3 " key={id}>
              <StaggerEffect id={idx}>
                <div className="bg-[#203A74]  p-6 xl:p-8 flex flex-col rounded-2xl shadow-xl  group hover:border-mainTextColor duration-700 hover:bg-white/60">
                  <div className="text-4xl text-s1 bg-white w-[53px] mb-5 rounded-full p-2 group-hover:text-mainTextColor duration-500">
                    {icon}
                  </div>
                  <h4 className="heading-4 group-hover:text-mainTextColor text-white pb-5">{name}</h4>
                  <p className=" text-white group-hover:text-mainTextColor pb-6">{desc}</p>
                  <Link
                    to={"/contact"}
